...
class LineAnalyzerDoc : public wxDocument {
 public:
  ...
  /// \brief Creates the active transmission line from an xml node.
  /// \param[in] node
  ///   The xml node.
  /// \param[in] line
  ///   The transmission line to populate.
  /// \return Success status.
  /// The transmission line will be created with valid references to the
  /// document shared data.
  bool CreateTransmissionLineFromXml(const wxXmlNode* node,
                                     TransmissionLine& line) const;

  /// \brief Creates transmission lines from an xml node.
  /// \param[in] node
  ///   The xml node.
  /// \param[in] lines
  ///   The transmission lines to populate.
  /// \return Success status.
  /// The transmission line will be created with valid references to the
  /// document shared data.
  bool CreateTransmissionLinesFromXml(const wxXmlNode* node,
